...
export default function User() {
    ...
    const Permissions = () => {
        ...
        if (resPermission.admin.access || resPermission.superAdmin.access) {
    ...
    const columns = [
        ...
        {
            name: 'Empresa',
            cell: (row) => <div onClick={() => { this.ShowUser(row.IdUser_PK); setModalUser(true) }} data-tag="allowRowEvents">{row.IdCompany_FK}</div>,
            selector: row => row.IdCompany_FK,
            sortable: true,
        },
        {
            name: 'Correo Electrónico',
            cell: (row) => <div onClick={() => { this.ShowUser(row.IdUser_PK); setModalUser(true) }} data-tag="allowRowEvents">{row.UserEmail}</div>,
        ...
        <div className="page-content py-3">
            ...
            <Modal show={ModalUser} onHide={() => { setModalUser(false) }} centered>
                <Form onSubmit={OnSubmitUser}>
                    ...
                    <Modal.Footer>
                        <Button type="submit" style={{ display: isView ? 'none' : 'inline-block' }} variant="success" onClick={() => { setModalUser(false) }}>
            ...
            <Modal show={ModalRole} onHide={() => { setModalRole(false) }} centered size="lg">
                <Form onSubmit={OnSubmitPermission}>
                    ...
                    <Modal.Footer>
                        <Button type="submit" variant="success" onClick={() => { setModalRole(false) }} >
                            <i className="bi bi-save me-2"></i>
                            Guardar
                        </Button>
                    </Modal.Footer>
